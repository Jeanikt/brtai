<template>
    ...
    <div v-else class="min-h-screen font-prompt">
        <!-- Header Público (sem autenticação) -->
        <header class="bg-white shadow-sm border-b border-gray-200">
            <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center h-16">
                    <!-- Logo -->
                    <Link href="/" class="flex items-center">
                    <ApplicationLogo fill="green" class="w-12 h-12" />
                    </Link>

                    <!-- Menu para usuários não autenticados -->
                    <div class="flex items-center gap-4">
                        <Link :href="route('login')" class="text-gray-700 hover:text-gray-900 font-medium">
                        Entrar
                        </Link>
                        <Link :href="route('register')"
                            class="bg-black text-white px-4 py-2 rounded-full font-semibold hover:bg-gray-800 transition-colors">
                        Criar Conta
                        </Link>
                    </div>
                </div>
